...
def resnet18(num_classes):
    return ResNet(block=BasicBlock, num_classes=num_classes, layers=[2,2,2,2])
...
def ResNetCifar(depth, num_classes):
    if depth == 18:
...
if __name__ == '__main__':
    ...
    model = ResNetCifar(18, 10)
    outputs = model(x)
    print('out size:{0}'.format(outputs.size()))
    flops, params = profile(model, (x,))
    print('flops:{0}. params:{1}'.format(flops, params))
    # print('model total params:{0}'.format(sum([params.nelement() for params in model.parameters()])))
